...
class PredicateFactory {
    ...
    private static func predicateForMap<T: NSManagedObject>(_ rect: MKMapRect, excluding objects: [T]) -> NSPredicate {
        ...
        let periodId = CoreDataProvider.selectedPeriod()?.stringDescription ?? ""

        return NSPredicate(format: "latitude <= %f AND longitude >= %f AND latitude >= %f AND longitude <= %f AND (NOT SELF IN %@) AND periodId == %@", topLat, topLong, bottomLat, bottomLong, objects, periodId)
    }
